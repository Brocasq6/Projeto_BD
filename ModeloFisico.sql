-- =============================================================
--  EvenSync — Script de Criação do Esquema (Modelo Lógico)
...
CREATE TABLE Inscricao (
    ...
    CONSTRAINT fk_insc_evento FOREIGN KEY (id_evento)
        ...
        ON UPDATE CASCADE ON DELETE RESTRICT,
    -- RC1: um participante só pode ter uma inscrição activa por evento
    CONSTRAINT uq_insc_part_evento UNIQUE (id_participante, id_evento),
...
CREATE TABLE Pagamento (
    ...
    CONSTRAINT uq_pag_inscricao UNIQUE (id_inscricao),
...
-- -------------------------------------------------------------
-- 9. Tem  (R5: Evento N—M Sessao)
-- -------------------------------------------------------------
CREATE TABLE Tem (
    id_evento INT NOT NULL,
    id_sessao INT NOT NULL,

    CONSTRAINT pk_tem  PRIMARY KEY (id_evento, id_sessao),
    CONSTRAINT fk_tem_evento  FOREIGN KEY (id_evento)
        REFERENCES Evento(id_evento)
        ON UPDATE CASCADE ON DELETE CASCADE,
    CONSTRAINT fk_tem_sessao  FOREIGN KEY (id_sessao)
        REFERENCES Sessao(id_sessao)
        ON UPDATE CASCADE ON DELETE CASCADE
);

-- -------------------------------------------------------------
-- 10. Apresentada  (R6: Sessao N—M Orador)
-- -------------------------------------------------------------
